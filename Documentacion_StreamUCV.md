...
Existen dos alternativas para lanzar la aplicación:

**Opción A: A través de código fuente (Terminal)**
Desde la misma terminal, inicie el programa ejecutando:
```bash
python main.py
```

**Opción B: A través del archivo ejecutable (Recomendado)**
Para facilitar el despliegue, se ha generado un archivo ejecutable autónomo.
1. Navegue a la carpeta `dist/` dentro del directorio del proyecto.
2. Haga doble clic en el archivo `main.exe` (o el nombre que se le haya asignado).
3. La aplicación gráfica se abrirá de forma nativa sin necesidad de abrir consolas de comandos.
